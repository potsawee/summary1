...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, input, u_len, w_len, target):
        ...
        return dec_output, enc_output_dict['gate_z'], enc_output_dict['u_output'], attn_scores, cov_scores
    ...
    def decode_beamsearch_adaptive_bias(self, adaptivebias, input, u_len, w_len, decode_dict):
        """
        this method is meant to be used at inference time
            input = input to the encoder
            u_len = utterance lengths
        ...
        y_out  = [torch.ones((1, time_step, vocab_size), dtype=torch.float).to(device) for _ in range(k)]
        ...
        for t in range(time_step-1):
            ...
            for i, beam in enumerate(beams):
                ...
                # decoder_output = self.decoder(beam[:,:t+1], s_output, s_len, logsoftmax=True)[:,-1,:]
                if t == 0:
                    decoder_output, beam_ht[i] = self.decoder.forward_step(beam[:,t:t+1], beam_ht[i], enc_output_dict, logsoftmax=False)
...
class HierarchicalGRU(nn.Module):
    def __init__(self, vocab_size, embedding_dim, rnn_hidden_size, num_layers, dropout, device):
        ...
        self.adapt_gru_ulevel = AdaptiveGRU(input_size=2*self.rnn_hidden_size, hidden_size=self.rnn_hidden_size,
                                            num_layers=num_layers, bias=True, batch_first=True, dropout=dropout, device=device)
    ...
    def forward(self, input, u_len, w_len):
        ...
        h0 = torch.zeros((1, self.rnn_hidden_size), dtype=torch.float).to(self.device)
        utt_output, segment_indices, gate_z = self.adapt_gru_ulevel(utt_input, h0, u_len)

        # segment level
        s_len = [len(x) for x in segment_indices]
        max_num_segments = max(s_len)

        seg_input = torch.zeros((batch_size, max_num_segments, self.rnn_hidden_size), dtype=torch.float).to(self.device)
        ...
        encoder_output_dict = {
            ...
            'u_output': utt_output, 'u_len': u_len, 'segment_indices': segment_indices, 'gate_z': gate_z,
...
class DecoderGRU(nn.Module):
    ...
    def forward(self, target, encoder_output_dict, logsoftmax=True):
        ...
        scores_s = F.log_softmax(scores_s, dim=-1)
        ...
        scores_su = torch.zeros(scores_u.shape).to(self.device)
        scores_su.fill_(float('-inf')) # when doing log-addition
        ...
        scores_suw.fill_(float('-inf')) # when doing log-addition

        # Segment -> Utterance
        for bn in range(batch_size):
            idx1 = 0
            idx2 = 0
            if segment_indices[bn][-1] == u_len[bn]-1:
                end_indices = segment_indices[bn]
            else:
                end_indices = segment_indices[bn] + [u_len[bn].item()-1]
            ...
            for i in range(len(segment_indices[bn])):
                ...
                scores_su[bn, :, i1:i2] = scores_s[bn, :, i].unsqueeze(-1) + F.log_softmax(scores_u[bn, :, i1:i2], dim=-1)
        # Utterance -> Word
        for bn in range(batch_size):
            idx1 = 0
            idx2 = 0
            ...
            for i in range(len(utt_indices[bn])):
                ...
                scores_suw[bn, :, i1:i2] = scores_su[bn, :, i].unsqueeze(-1) + F.log_softmax(scores_w[bn, :, i1:i2], dim=-1)

        # scores_suw = F.normalize(scores_suw, p=1, dim=-1)
        # scores_suw = F.softmax(scores_suw, dim=-1)
        scores_suw = torch.exp(scores_suw)
    ...
    def forward_step(self, xt, ht, encoder_output_dict, logsoftmax=True):
        ...
        for bn, l in enumerate(s_len):
            scores_s[bn,:,l:].fill_(float('-inf'))
        scores_s = nn.functional.log_softmax(scores_s, dim=-1)

        # attention mechanism LEVEL --- Utterance (u)
        scores_u = torch.bmm(rnn_output, self.attention_u(u_output).permute(0,2,1))
        for bn, l in enumerate(u_len):
            scores_u[bn,:,l:].fill_(float('-inf'))
        scores_u = nn.functional.log_softmax(scores_u, dim=-1)
        scores_su = torch.zeros(scores_u.shape).to(self.device)
        scores_su.fill_(float('-inf')) # when doing log-addition
        ...
        for bn, l in enumerate(w_len):
            scores_w[bn,:,l:].fill_(float('-inf'))
        scores_w = nn.functional.log_softmax(scores_w, dim=-1)
        scores_suw = torch.zeros(scores_w.shape).to(self.device)
        scores_suw.fill_(float('-inf')) # when doing log-addition
        ...
        for bn in range(batch_size):
            ...
            for i in range(len(utt_indices[bn])):
                ...
                i2 = end_indices[i]+1 # python
                scores_suw[bn, :, i1:i2] = scores_su[bn, :, i].unsqueeze(-1) + scores_w[bn, :, i1:i2]
        # scores_suw = nn.functional.normalize(scores_suw, p=1, dim=-1)
        scores_suw = nn.functional.softmax(scores_suw, dim=-1)
        import pdb; pdb.set_trace()
        ...
        return dec_output[:,-1,:], ht1, scores_suw
